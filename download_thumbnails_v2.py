...
def get_yt_dlp_options(**extra: Any) -> dict[str, Any]:
    ...
    if _is_node_available():
        # ИСПРАВЛЕНО: Новый формат для актуальных версий yt-dlp
...
def _safe_filename(title: str, index: int) -> str:
    """Create a filesystem-safe filename like '01_my_video.jpg'."""
    safe = re.sub(r"[^\w\-. ]", "", title).strip()[:65]
    # collapse spaces
    safe = re.sub(r"\s+", "_", safe)
    return f"{index:02d}_{safe}.jpg"
...
def _fetch_video_infos(channel_url: str, num_videos: int) -> list[dict[str, Any]]:
    ...
    output_tpl = os.path.join(safe_channel, "%(id)s_%(title)s")

    ydl_opts = get_yt_dlp_options(
        playlist_items=f"1-{num_videos}",
        outtmpl=output_tpl + ".%(ext)s",
    )
    ...
    # Ensure we have full descriptions by re-fetching per-video when empty.
    result: list[dict[str, Any]] = []
    channel_name = _channel_name_from_url(channel_url)
    ...
    for i, video in enumerate(videos_raw, start=1):
        ...
        thumb_opts = get_yt_dlp_options(
            outtmpl=os.path.join(output_folder, f"{i:02d}_{_safe_filename(title, i).rsplit('.', 1)[0]}"),
            skip_download=True,
            writethumbnail=True,
        )
...
def download_channel_thumbnails(channel_url: str, num_videos: int = 12) -> None:
    ...
    # Warn once if Node.js is unavailable.
